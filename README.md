...
- Default layout is **Tape rails**: dual skinny rails (them left, you right), names and scores above each roster, no framed card. Type scales to the allocated widget. Score ticks flash lime `+N` or alert red `-N` on the pts cell, then settle. **RedZone** and **Ticket** remain as watch templates.
