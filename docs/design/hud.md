...
- Ticker: `ticker.nfl` is a thin bottom NFL score strip. Overlay type stays a compact ticker (`hud-type-ticker`), not a headline. Studio treats it as one selectable block. No ON AIR wordmark.
