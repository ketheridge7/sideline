...
describe('overlayEspnMatchup', () => {
  ...
  it('lets compact mLiveScoring move HUD totals down when preferLive is set', () => {
    ...
    expect(overlayEspnMatchup(highPrev, live, 1)?.myPoints).toBe(18)
  ...
  it('uses in-progress week-2 totalPointsLive instead of leftover week-1 header totals', () => {
    const week1Finals = {
      ...prev,
      myPoints: 115.26,
      oppPoints: 122.22,
      starters: [
        { playerId: '106', name: 'LaPorta', position: 'TE', nflTeam: 'DET', points: 0 },
        { ...prev.starters[0], points: 24.1 }
      ],
      oppStarters: [
        { playerId: '201', name: 'Bates', position: 'K', nflTeam: 'CIN', points: 0 },
        { ...prev.oppStarters[0], points: 28.4 }
      ],
      scoresFinal: false
    }
    const live = {
              ...
              entries: [
                {
                  ...
                  playerId: 106,
                  playerPoolEntry: {
                    player: { fullName: 'LaPorta', defaultPositionId: 3 }
                  }
                },
                {
                  lineupSlotId: 0,
                  playerId: 100,
                  playerPoolEntry: {
                    player: { fullName: 'Hurts', defaultPositionId: 1 }
                  }
                }
              ]
            }
              ...
              entries: [
                {
                  ...
                  playerId: 201,
                  playerPoolEntry: {
                    player: { fullName: 'Bates', defaultPositionId: 5 }
                  }
                },
                {
                  lineupSlotId: 0,
                  playerId: 200,
                  playerPoolEntry: {
                    player: { fullName: 'Mahomes', defaultPositionId: 1 }
                  }
                }
              ]
            }
    ...
    const next = overlayEspnMatchup(week1Finals, live, 2)
    expect(next?.myPoints).toBe(17.2)
    expect(next?.oppPoints).toBe(3)
    expect(next?.starters.find((player) => player.playerId === '106')?.points).toBe(17.2)
    expect(next?.oppStarters.find((player) => player.playerId === '201')?.points).toBe(3)
