...
describe('layoutFromPreset', () => {
  ...
  it('Tape rails keep dual skinny rails with you left, them right, names and scores above each', () => {
    ...
    expect(layout.widgets.find((row) => row.id === 'score.opp')?.w).toBe(
      layout.widgets.find((row) => row.id === 'team.opp.name')?.w
    )
    expect(layout.widgets.find((row) => row.id === 'toast.slot')?.hidden).toBe(true)
    expect(vis.every((row) => row.opacity === 0)).toBe(true)
    ...
    expect(layout.widgets.find((row) => row.id === 'col.mine.name')?.opacity).toBe(0)
...
describe('parseOverlayLayout', () => {
  ...
  it('merges saved widget positions onto the named preset', () => {
    ...
    expect(parsed.widgets.find((row) => row.id === 'score.opp')?.x).toBe(80.8)
  })
